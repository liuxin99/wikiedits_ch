...
class EditFilter(object):
    ...
    def __init__(self,
        ...
        self.segmenter = self.__ssplit if lang == "chinese" \
                    else nltk.data.load('tokenizers/punkt/%s.pickle' % lang)
    ...
    def __looks_like_text_edition(self, old_text, new_text): # 判断old_text与new_text是否有编辑操作
    ...
    def __looks_like_sentence_edition(self, old_sent, new_sent): # 判断句子对是否能进行编辑检测，能的话计算并返回编辑距离
        if old_sent == new_sent:
            log.info("sentences are equal")
            return False
        
        if self.lang == "chinese":
            # the number of words in a sentence is obtained by counting the lenth of sentence
            counts = [len(old_sent), len(new_sent)]
        else:
    ...
    def __levenshtein_ratio(self, old_sent, new_sent):
        if self.lang == "chinese":  # if chinese, no need to split sentence with space
            min_words = min(len(old_sent), len(new_sent))
            dist = self.__levenshtein_on_words(old_sent, new_sent)
        else:
    ...
    def __levenshtein_on_words(self, words1, words2): #在词级别计算编辑距离, 中文则在字级别
        if self.lang == "chinese": # if chinese, compute Levenshtein distance directly
